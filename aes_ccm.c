...
0xd2,0xb1,0x10,0xce,0x84,0xf4,0x4b,0xd5,0x2a,0x35,0x6a,0x7c,0x2d,0x67,0x27,0xdb,
0x40,0x65,0xab,0xae,0xc5,0x11,0xcb,0x1a,0xf5,0x22,0x41,0x54,0xe6,0x1a,0xdc,0x23,
0xfc,0x46,0xb3,0x05
};
...
//    NRF_LOG_HEXDUMP_INFO(p, sizeof(p));
    ...
    nrf_aes_ecb_encrypt(aes_key, plain, output);

//    NRF_LOG_INFO("Cipher Text:\n");
//    NRF_LOG_HEXDUMP_INFO(c, 16);
}

#define LEN sizeof(plain)
void aes_ccm_test(void)
{

    aes_ccm_encrypt(aes_key, nonce, adata, sizeof(adata), output+LEN, 4, plain, LEN, output);
//	aes_ccm_decrypt(aes_key, nonce, adata, sizeof(adata), output+LEN, 4, output, LEN, output);

//    NRF_LOG_INFO("Clear Text:\r\n");
//    NRF_LOG_RAW_HEXDUMP_INFO(plain, LEN);
//    NRF_LOG_INFO("Cipher Text:\r\n");
//    NRF_LOG_RAW_HEXDUMP_INFO(output, LEN+4);

	NRF_LOG_INFO("AES128-CCM TEST: ");
	if(memcmp(output, cipher, LEN+4) == 0) {
